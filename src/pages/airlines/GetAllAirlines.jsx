...
import UpdateAirport from "./UpdateAirlines";
import UpdateAirlines from "./UpdateAirlines";
...
const GetAllAirlines = () => {
  ...
  const getAllRoomData = async () => {
    try {
      const response = await axios.get("http://localhost:3000/airlines/all", {
        headers: {
  ...
  const deleteAirline = async (airline_id) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3000/airlines/${airline_id}/delete`,
        {},
        {
          headers: {
            "Content-Type": "application/json",
      ...
      <MasterLayout>
        <div className="card">
          <div className="card-header">
            <h5 className="card-title mb-0">All the Available Airlines</h5>
          </div>
          {/* card body start */}
          <div className="card-body">
            <div className="table-responsive">
              <table className="table basic-border-table mb-0">
                ...
                <tbody>
                  {roomData.length > 0 ? (
                    roomData.map((airline) =>
                      airline.flights.length > 0 ? (
                        airline.flights.map((flight, index) => (
                          <tr key={index}>
                            {index === 0 && (
                              <>
                                <td rowSpan={airline.flights.length}>
                                  {airline.airline_id}
                                </td>
                                <td rowSpan={airline.flights.length}>
                            ...
                            {index === 0 && (
                              <td rowSpan={airline.flights.length}>
                                ...
                                <button
                                  ...
                                  onClick={() => delete airline.airline_id}
                                >
                                  Delete
                                </button>
                              </td>
                        ...
                        <tr key={airline.airline_id}>
                          ...
                          <td>
                            ...
                            <button
                              className="btn btn-danger btn-sm"
                              onClick={() => deleteAirline(airline.airline_id)}
                            >
                              Delete
                            </button>
                          </td>
