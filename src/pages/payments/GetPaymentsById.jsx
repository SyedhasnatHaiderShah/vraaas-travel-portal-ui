  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/payments/${formatedData.payment_id}`,
        {
          headers: {
    ...
    <div className=" w-100">
      <MasterLayout>
        <div className=" w-100 border p-3 ">
          ...
          {/* <HotelTableAllData /> */}
          <h3>Get Payment by ID</h3>
          <form action="#" onSubmit={handleSubmit(onSubmit)}>
            <div className=" col-12">
              <label className="form-label">Enter Payment ID</label>
                      ...
                      <tbody>
                        {bookingData ? (
                          <tr>
                            ...
                            <td>{bookingData.payment_method}</td>
                            <td>{bookingData.status}</td>
                          </tr>
                        ) : (
                          <tr>
                            <td colSpan="6" className="text-center">
                              No data available. Please submit a valid Payment
                              ID.
                            </td>
                          </tr>
                        )}
